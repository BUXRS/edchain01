...
export async function POST(
  ...
  try {
    ...
    // Update issuer with wallet address (normalized to lowercase to avoid duplicate rows)
    ...
    try {
      const universities = await sql`
        SELECT name, email, wallet_address FROM universities WHERE id = ${issuer.university_id}
      `
