/**
 * University-scoped revoker: DELETE = soft delete (is_active = false, status = 'inactive').
 * Call after revoking on-chain if the revoker was active.
 */
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Revoker removed" })
  } catch (error) {
    console.error("[UniversityRevokersDelete] Error:", error)
    return NextResponse.json({ error: "Failed to remove revoker" }, { status: 500 })
  }
}
