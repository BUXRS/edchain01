/**
 * University-scoped verifier: DELETE = soft delete (is_active = false, status = 'inactive').
 * Call after removing on-chain (removeVerifier) if the verifier was active.
 */
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Verifier removed" })
  } catch (error) {
    console.error("[UniversityVerifiersDelete] Error:", error)
