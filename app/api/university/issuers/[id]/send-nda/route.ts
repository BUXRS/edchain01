...
import { sendIssuerNdaReminderEmail } from "@/lib/services/email-service"
...
export async function POST(
  ...
  try {
    ...
    const result = await sendIssuerNdaReminderEmail({
      to: row.email,
    ...
    return NextResponse.json({ success: true, message: "NDA reminder email sent." })
  } catch (e) {
    console.error("[SendNda] Error:", e)
    return NextResponse.json({ error: "Failed to send NDA email" }, { status: 500 })
  }
}
